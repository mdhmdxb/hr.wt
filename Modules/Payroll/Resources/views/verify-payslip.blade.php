...
<div class="max-w-md mx-auto bg-white dark:bg-slate-800 rounded-xl shadow p-6 text-center">
    @if($valid)
        ...
        <dl class="text-left space-y-2 border-t border-slate-200 dark:border-slate-600 pt-4">
            <div><dt class="text-xs text-slate-500 dark:text-slate-400">Employee</dt><dd class="font-medium text-slate-900 dark:text-slate-100">{{ $payslip->employee->full_name ?? '—' }}</dd></div>
            <div><dt class="text-xs text-slate-500 dark:text-slate-400">Period</dt><dd class="font-medium text-slate-900 dark:text-slate-100">{{ $payslip->payrollRun->period_start->format('M Y') }}</dd></div>
            <div><dt class="text-xs text-slate-500 dark:text-slate-400">Total net salary</dt><dd class="font-medium text-slate-900 dark:text-slate-100">{{ number_format($payslip->net_pay ?? 0, 2) }}</dd></div>
        </dl>
    @else
