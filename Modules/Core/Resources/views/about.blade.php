...
<div class="max-w-2xl mx-auto">
    <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-lg border border-slate-200/50 dark:border-slate-700/50 p-8 md:p-10 overflow-hidden">
        <div class="flex flex-col sm:flex-row items-center sm:items-start gap-6 mb-6">
            @if(!empty($logoPath))
            <img src="{{ $logoPath }}" alt="{{ $appName }}" class="w-24 h-24 sm:w-28 sm:h-28 object-contain flex-shrink-0 rounded-xl border border-slate-200 dark:border-slate-600">
            @endif
            <div class="flex-1 text-center sm:text-left">
                <h2 class="wise-heading text-2xl font-bold text-slate-900 dark:text-slate-100">{{ $appName }}</h2>
                <p class="text-slate-500 dark:text-slate-400 text-sm font-medium mt-1">Version {{ $version }}</p>
            </div>
        </div>
