...
@section('content')
<form method="POST" action="{{ route('holidays.update', $holiday) }}" class="max-w-xl space-y-4">
    @csrf
    @method('PUT')
    <div class="bg-white dark:bg-slate-800 rounded-xl shadow p-6 space-y-4">
        <div>
            <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Name</label>
            <input type="text" name="name" value="{{ old('name', $holiday->name) }}" required placeholder="e.g. National Day"
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                ...
                <input type="date" name="date" value="{{ old('date', $holiday->date->format('Y-m-d')) }}" required class="w-full rounded-lg border border-slate-300 dark:border-slate-600 dark:bg-slate-700 dark:text-white px-3 py-2">
            ...
            <div>
                ...
                <input type="date" name="end_date" value="{{ old('end_date', $holiday->end_date ? $holiday->end_date->format('Y-m-d') : '') }}" class="w-full rounded-lg border border-slate-300 dark:border-slate-600 dark:bg-slate-700 dark:text-white px-3 py-2">
        ...
        <div>
            ...
            <input type="text" name="country_code" value="{{ old('country_code', $holiday->country_code) }}" placeholder="e.g. AE" maxlength="10"
                class="w-full rounded-lg border border-slate-300 dark:border-slate-600 dark:bg-slate-700 dark:text-white px-3 py-2">
        </div>
    </div>
    <div class="flex gap-3">
        ...
        <button type="submit" class="px-6 py-2 wise-btn text-white rounded-lg">Update Holiday</button>
    </div>
</form>
@endsection
