...
@section('heading', 'Add Site')

@section('content')
<form method="POST" action="{{ route('site.store') }}" class="max-w-xl space-y-4">
    @csrf
    <div class="bg-white dark:bg-slate-800 rounded-xl shadow p-6 space-y-4">
        <div>
            ...
            <select name="branch_id" required class="w-full rounded-lg border border-slate-300 dark:border-slate-600 dark:bg-slate-700 dark:text-white px-3 py-2">
                @foreach($branches as $b)
                    <option value="{{ $b->id }}" {{ old('branch_id') == $b->id ? 'selected' : '' }}>{{ $b->name }} ({{ $b->company->name ?? '' }})</option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <input type="text" name="name" value="{{ old('name') }}" required placeholder="e.g. Building A, Warehouse"
