...
@section('content')
<form method="POST" action="{{ route('department.update', $department) }}" class="max-w-xl space-y-4">
    @csrf
    @method('PUT')
    <div class="bg-white dark:bg-slate-800 rounded-xl shadow p-6 space-y-4">
        <div>
            ...
            <select name="branch_id" required class="w-full rounded-lg border border-slate-300 dark:border-slate-600 dark:bg-slate-700 dark:text-white px-3 py-2">
                @foreach($branches as $b)
                    <option value="{{ $b->id }}" {{ old('branch_id', $department->branch_id) == $b->id ? 'selected' : '' }}>{{ $b->name }}@if($b->company) ({{ $b->company->name }})@endif</option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <input type="text" name="name" value="{{ old('name', $department->name) }}" required placeholder="e.g. HR, IT, Finance"
        ...
        <div class="flex gap-2">
            <button type="submit" class="px-4 py-2 wise-btn text-white rounded-lg">Update Department</button>
